...
export class GeminiLLMProvider implements ILLMProvider {
    ...
    async generateSemanticMutants(context: IMutationContext): Promise<ISemanticMutant[]> {
    ...
    async generateDefenseTests(mutants: ISemanticMutant[], context: IMutationContext): Promise<string> {
    ...
    private createSemanticMutationPrompt(context: IMutationContext): string {
        return `
You are DefMutator, an expert in defense system testing. Analyze this ${context.functionType} system code and generate realistic semantic mutations that simulate actual defense system failures.
...
`;
    ...
    private createDefenseTestPrompt(mutants: ISemanticMutant[], context: IMutationContext): string {
        const mutantDescriptions = mutants.map(m => `- ${m.description} (${m.riskLevel} risk)`).join('\n');
...
`;
    ...
    private parseAnalysisResult(response: string, originalCode: string): IMutationContext {
        try {
            const jsonMatch = response.match(/\{[\s\S]*\}/);
            if (!jsonMatch) {
